...
using WPFT = System.Windows.Threading;
...
namespace MCSkinn
{
    public partial class Editor : Form
    {
        ...
        public void Initialize(Language language)
        {
            ...
            //Renderer.ManipulationInertiaStarting += Renderer_ManipulationInertiaStarting;
        ...
        private void Renderer_ManipulationCompleted(object? sender, WPF.Input.ManipulationCompletedEventArgs e)
        {
        ...
        private void Renderer_ManipulationDelta(object? sender, WPF.Input.ManipulationDeltaEventArgs e)
        {
            if (!GlobalSettings.IsManipulationEnabled)
            {
                e.Cancel();
        ...
        private void Renderer_ManipulationStarting(object? sender, WPF.Input.ManipulationStartingEventArgs e)
        {
            if (!GlobalSettings.IsManipulationEnabled)
            {
                e.Cancel();
            ...
            using (var backup = new ColorGrabber(GlobalDirtiness.CurrentSkin, skin.Width, skin.Height))
            {
                ...
                try
                {
                    if (_mouseIsDown || isTouchDown)
                    {
                        ...
                        if (e.ChangedButton == WPF.Input.MouseButton.Left && !isTouchDown)
                        {
                            ...
                            _tools[(int)Tools.Camera].Tool.EndClick(currentSkin, _lastSkin, e.GetPosition(Renderer).W2D(), e.ChangedButton);
                    }
                }
                catch (Exception ex)
                {
        ...
        private void Renderer_MouseMove(object sender, WPF.Input.MouseEventArgs e)
        {
            ...
            using (var backup = new ColorGrabber(GlobalDirtiness.CurrentSkin, skin.Width, skin.Height))
            {
                ...
                try
                {
                    if (_mouseIsDown || isTouchDown)
                    {
                        if (e.LeftButton == WPF.Input.MouseButtonState.Pressed && (!isTouchDown || !GlobalSettings.StylusToDraw))
                        {
                            ...
                            _tools[(int)Tools.Camera].Tool.MouseMove(_lastSkin, e.GetPosition(Renderer).W2D());
                    }
            ...
            using (var backup = new ColorGrabber(GlobalDirtiness.CurrentSkin, skin.Width, skin.Height))
            {
                ...
                try
                {
                    if (e.ChangedButton == WPF.Input.MouseButton.Left && !isTouchDown)
                    {
                        ...
                        _tools[(int)Tools.Camera].Tool.BeginClick(_lastSkin, e.GetPosition(Renderer).W2D(), e.ChangedButton);
                }
                catch (Exception ex)
                {
                    backup.Save();
